...
3. Run one_step on sensing 0 only (tells[0:1], strike[0:1]) with alpha = 0.2, seed = 1,
and hidden_size = 4. Print:
• layer_2 before and after the step
• layer_2_error before and after
• the shapes of both weight matrices before and after

4. Hand-verify: pick one entry of weights_1_2 after the step. On paper or in a comment, show
the calculation:
new = old - alpha * layer_1.T.dot(layer_2_delta) at that index.

5. Comment block: explain in your own words why layer_1_delta uses weights_1_2.T (not
